...
if (!fs.existsSync(manifestPath)) {
  ...
  if (parsed && (!manifest || typeof manifest !== 'object' || !manifest.name || typeof manifest.name !== 'string')) {
...
if (!hasComponent) {
  errors.push('No components found — add at least one (' + COMPONENT_DIRS.map((d) => d + '/').join(', ') + ') at the repo root.');
}
